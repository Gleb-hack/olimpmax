  ...
  return <><Header title="Мой план" />
    ...
    {plan.isPending ? <Loading label="Загружаем ваш план…" /> : plan.isError ? <EmptyState icon={ClipboardList} title="План пока недоступен" action={<Button onClick={() => { plan.refetch(); events.refetch(); }}>Попробовать снова</Button>}>{plan.error.message}</EmptyState> : <>
      ...
      {!entries.length ? <EmptyState icon={CalendarDays} title="Большие планы начинаются здесь" action={<Link className="button-link" to="/catalog">Найти олимпиаду</Link>}>Сохраните интересные олимпиады из каталога — они появятся в вашем плане.</EmptyState> : <>
        ...
        <section className="section"><h2 className="section-caption">Ближайшие этапы · 90 дней</h2>
          {events.isPending ? <Loading /> : shownEvents.length ? <div className="plan-card-list">{shownEvents.map(event => <PlanCard key={`${event.stageId}-${event.kind}`} entry={shownById.get(event.olympiadId)!} event={event} onOpen={() => setEditingId(event.olympiadId)} />)}</div> : !events.isError && <div className="subtle-empty"><CalendarDays size={21} /><p>Проверенных дат пока нет<span>Уточняйте расписание у организатора. Сохранённые олимпиады остаются в плане.</span></p></div>}
        </section>
